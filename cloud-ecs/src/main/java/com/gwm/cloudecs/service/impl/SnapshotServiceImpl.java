...
public class SnapshotServiceImpl implements SnapshotService {
    ...
    public ResponseResult snapshotList(SnapshotListDTO snapshotListDTO) {
        ...
        try {
            ...
            for (SnapshotListVO snapshot : list) {
                String s = HttpClient.get(String.format(url, String.format("/bcp/v2/snapshot/status?snapshot_id=%s&region_name=%s", snapshot.getUuid(), snapshot.getRegion())));
    ...
    public ResponseResult snapshotInfo(String snapshotId, String regionName) {
        JSONObject snapshotDate = HttpClient.getEcs(String.format(url, String.format("/bcp/v2/snapshot/detail?snapshot_id=%s&region_name=%s", snapshotId, regionName)));
    ...
    public ResponseResult snapshotCreate(SnapshotDTO snapshotDTO) {
        try {
            ...
            JSONObject jsonObject1 = HttpClient.postEcs(String.format(url, "/bcp/v2/snapshot/create"), jsonObject.toJSONString());
    ...
    public ResponseResult snapshotUpdate(SnapshotUpdateDTO snapshotUpdateDTO) {
        try {
            ...
            HttpClient.postEcs(String.format(url, "/bcp/v2/snapshot/update"), jsonObject.toJSONString());
    ...
    public ResponseResult snapshotDelete(SnapshotDeleteDTO snapshotDeleteDTO) {
        try {
            Snapshot snapshot = snapshotMapper.selectBySnapshotId(snapshotDeleteDTO.getSnapshotId());
            JSONObject jsonObject = new JSONObject() {{
                put("snapshot_id", snapshotDeleteDTO.getSnapshotId());
                put("region_name", snapshotDeleteDTO.getRegion());
            }};
            HttpClient.postEcs(String.format(url, "/bcp/v2/snapshot/delete"), jsonObject.toJSONString());
