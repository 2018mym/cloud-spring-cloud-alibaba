    ...
    public ResponseResult createSshkeyInfo(String ticket, KeyPairs keyParis){
        ...
        try {
           ...
           // JSONObject  sshKeyData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/sshkey/create"), jsonObject.toJSONString());
            String s = HttpClient.post(String.format(GlobalVarConfig.ecsUrl,"/bcp/v2/sshkey/create"), jsonObject.toJSONString());
    ...
    public ResponseResult delSshkeyInfo(String ticket, List<Integer> ids)  throws Exception {
        if(ids!=null&&ids.size()>0) {
            ids.forEach(id-> {
                ...
                try {
                    String s =  HttpClient.post(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/sshkey/delete"), jsonObject.toJSONString());
                    JSONObject parse = (JSONObject) JSONObject.parse(s);
