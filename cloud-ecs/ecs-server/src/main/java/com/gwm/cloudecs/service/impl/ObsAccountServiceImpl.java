...
public class ObsAccountServiceImpl  implements ObsAccountService {
    ...
    public ResponseResult addAccountInfo(ObsAccount account)  {
        ...
        obsParam.put("quota_type", "user");
         //调ceph开通账号
        //JSONObject escData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl,"/v1/createAK"), obsParam.toJSONString());
            JSONObject escData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl,"/bcp/v2/rgw/createAK"), obsParam.toJSONString());
        ...
        if(escData!=null) {
            ...
            account.setCreateAt(DateUtil.format(new Date(), "yyyy-MM-dd HH:mm:ss"));//创建时间
    ...
    public ResponseResult updateCloudAccountInfo(ObsAccount account) {
        ...
        try {
            ...
            JSONObject cephData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/rgw/updateAK"), obsParam.toJSONString());
            //String server = escData.getString("data");
            if (cephData != null) {
                oldEntity1.setUpdateAt(DateUtil.format(new Date(), "yyyy-MM-dd HH:mm:ss"));//更新时间
    ...
    public ResponseResult updateCloudAccountQuota(ObsAccount account) {
        try {
            ...
            if(oldEntity1.getQuota()>account.getQuota()){
                ...
                JSONObject cephData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/rgw/set_quota"), obsParam.toJSONString());
        ...
        try {
            if(ids!=null&&ids.size()>0) {
                for(Integer id:ids){
                    ...
                    JSONObject cephData = HttpClient.postEcs(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/rgw/delAK"), obsParam.toJSONString());

                    ObsAccount account = obsAccountMapper.selectByPrimaryKey(id1);
                    account.setDeleted("1"); //被删除了
                    account.setDeletedAt(DateUtil.format(new Date(), "yyyy-MM-dd HH:mm:ss"));
    ...
    public ResponseResult getBucket(ObsAccount account){
        ...
        ObsAccount oldEntity1 = obsAccountMapper.selectByPrimaryKey(account.getId());
        try {
            String s = HttpClient.get(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/rgw/bucket_size?uid="+oldEntity1.getCephUser() ));
            ...
            String s1 = HttpClient.get(String.format(GlobalVarConfig.ecsUrl, "/bcp/v2/rgw/get_quota?uid="+oldEntity1.getCephUser()+"&quota_type=user"));
