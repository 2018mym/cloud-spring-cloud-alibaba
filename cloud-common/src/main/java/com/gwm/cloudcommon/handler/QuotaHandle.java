    ...
    public void putQuotas(String ticket, String operate, String resourceCode, Integer count)  {
        try {
            ...
            if (200 <= responseResult.getCode() && responseResult.getCode() < 300) {
                log.info(String.format(String.format("QuotaHandle中处理配额完成，配额参数：operate:%s,resourceCode:%s,count:%s", operate, resourceCode, count)));
            } else {
                log.error(String.format(String.format("QuotaHandle中处理配额异常，配额参数：operate:%s,resourceCode:%s,count:%s", operate, resourceCode, count)));
 ...
 /**
    ...
    public void putQuotas(String ticket, String operate, Map<String, Integer> resourceCodeCount)  {
        try {
            ...
            ResponseResult responseResult = JSON.parseObject(responseBody, ResponseResult.class);
            if (200 <= responseResult.getCode() && responseResult.getCode() < 300) {
                log.info(String.format(String.format("QuotaHandle中处理配额完成，配额参数：operate:%s,resourceCodeCount:%s", operate, resourceCodeCount.toString())));
            } else {
                log.error(String.format(String.format("QuotaHandle中处理配额异常，配额参数：operate:%sresourceCodeCount:%s", operate, resourceCodeCount.toString())));
                throw new CommonCustomException(responseResult.getCode(), responseResult.getMessage());
            }
        } catch (Exception e) {
            log.error(String.format(String.format("QuotaHandle---try---catch---异常，配额参数：operate:%sresourceCodeCount:%s", operate, resourceCodeCount.toString())));
