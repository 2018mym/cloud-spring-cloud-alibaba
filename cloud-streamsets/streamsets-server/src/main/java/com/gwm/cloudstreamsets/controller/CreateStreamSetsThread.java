            ...
            if(streamSets.getInstallState().equals("INIT")){ // ECS需要监控状态的
                try{
                   if(getECSState(streamSets)){  //  为running 状态，查询IP
                            ...
                            if(null!=ip){
                                // 注册dop
                                JSONObject host =  streamSetsService.searchHost(ip);
                                ...
                                if(null == hostId){
                                    ...
                                    tmp.setHost(ip);
                                    streamSetsService.transferHostToIdlemodule(host.getInteger("bkBizId"),tmp.getHostId());
                                    streamSetsService.transferHostToResourcemodule(host.getInteger("bkBizId"),tmp.getHostId());
                                    streamSetsService.deleteHost(tmp,host.getString("bkBizId"));
                                    //再增加
                                    ResponseResult result =  streamSetsService.addHostToResource(ip);
                                    hostId =  streamSetsService.searchHostId(ip);
                                }
                ...
                try {
                    ...
                    log.info("查询task："+streamSets.getTaskId()+"状态！");
                } catch (IOException e) {
                    e.printStackTrace();
                }
